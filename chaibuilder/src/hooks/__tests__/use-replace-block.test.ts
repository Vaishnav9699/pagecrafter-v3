...
import { replaceBlock, useReplaceBlock } from "@chaibuilder/hooks/use-replace-block";
import { ChaiBlock } from "@chaibuilder/types/common";
import { act, renderHook } from "@testing-library/react";
import { beforeEach, describe, expect, it, vi } from "vitest";

// Mock dependencies
vi.mock("@chaibuilder/hooks/history/use-blocks-store-undoable-actions", () => ({
  useBlocksStore: vi.fn(),
  useBlocksStoreUndoableActions: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-selected-blockIds", () => ({
  useSelectedBlockIds: vi.fn(),
}));

vi.mock("@chaibuilder/core/main", () => ({
  PERMISSIONS: {
