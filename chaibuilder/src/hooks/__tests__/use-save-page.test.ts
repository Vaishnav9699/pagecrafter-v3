...
import { partialBlocksAtom } from "@chaibuilder/hooks/partial-blocks/atoms";
import { builderSaveStateAtom, checkMissingTranslations, useSavePage } from "@chaibuilder/hooks/use-save-page";
import { getRegisteredChaiBlock } from "@chaibuilder/runtime";
import { act, renderHook } from "@testing-library/react";
import { beforeEach, describe, expect, it, vi } from "vitest";

vi.mock("@chaibuilder/runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@chaibuilder/runtime")>();
  return {
    ...actual,
...
vi.mock("@chaibuilder/hooks/use-builder-prop", () => ({
  useBuilderProp: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-get-page-data", () => ({
  useGetPageData: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-permissions", () => ({
  usePermissions: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-languages", () => ({
  useLanguages: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-is-page-loaded", () => ({
  useIsPageLoaded: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-check-structure", () => ({
  useCheckStructure: vi.fn(),
}));

vi.mock("@chaibuilder/hooks/use-theme", () => ({
  useTheme: vi.fn(),
...
describe("useSavePage - prevent save when no unsaved changes", () => {
  ...
  beforeEach(async () => {
    ...
    const { useBuilderProp } = await import("@chaibuilder/hooks/use-builder-prop");
    const { useGetPageData } = await import("@chaibuilder/hooks/use-get-page-data");
    const { usePermissions } = await import("@chaibuilder/hooks/use-permissions");
    const { useLanguages } = await import("@chaibuilder/hooks/use-languages");
    const { useIsPageLoaded } = await import("@chaibuilder/hooks/use-is-page-loaded");
    const { useCheckStructure } = await import("@chaibuilder/hooks/use-check-structure");
...
describe("useSavePage - getAllPartialIds", () => {
  ...
  beforeEach(async () => {
    ...
    const { useBuilderProp } = await import("@chaibuilder/hooks/use-builder-prop");
    const { useGetPageData } = await import("@chaibuilder/hooks/use-get-page-data");
    const { useTheme, useThemeOptions } = await import("@chaibuilder/hooks/use-theme");
    const { usePermissions } = await import("@chaibuilder/hooks/use-permissions");
    const { useLanguages } = await import("@chaibuilder/hooks/use-languages");
    const { useIsPageLoaded } = await import("@chaibuilder/hooks/use-is-page-loaded");
    const { useCheckStructure } = await import("@chaibuilder/hooks/use-check-structure");
